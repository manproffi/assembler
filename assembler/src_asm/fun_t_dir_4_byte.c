...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/18 19:47:05 by sprotsen          #+#    #+#             */
/*   Updated: 2017/06/18 19:47:07 by sprotsen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** in - for_all_opcode.c
** valid_str(char *str, int i, t_algo *lst) - fun_t_ind.c
*/

int		with_label_t_dir_4_byte(t_algo *lst, char *str, t_algo *list)
{
	int				i;
	char			*label_name;
...
int		with_number_t_dir_4_byte(t_algo *lst, char *str)
{
	int				i;
	unsigned int	a;
...
int		parsing_t_dir_4_byte_v2(t_algo *lst, char *str, t_algo *list)
{
	...
	{
		if (with_label_t_dir_4_byte(lst, str + i + 2, list) == 0)
			return (FALSE);
	}
	else if (str[i] == DC && (ft_isdigit(str[i + 1]) || str[i + 1] == '-'))
	{
		if (with_number_t_dir_4_byte(lst, str + i + 1) == FALSE)
			return (FALSE);
	}
	else
		return (error_message(lst, NULL));
